...
},{"./canvas-spec.coffee":43,"./keyboard-state-spec.coffee":44}],2:[function(require,module,exports){
/*!
...
},{"./fn.coffee":38,"./system.coffee":40}],40:[function(require,module,exports){
var System, _,
  __slice = [].slice;
...
},{"./fn.coffee":38}],41:[function(require,module,exports){
...
describe('Canvas2D', function() {
  ...
  it('can be loaded as component data via getData', function() {
    return expect(canvas).to.exist;
  });
  it('has a canvas element and rendering context and camera', function() {
    expect(canvas.canvas).to.be["instanceof"](HTMLElement);
    expect(canvas.g2d).to.be["instanceof"](CanvasRenderingContext2D);
    return expect(canvas.camera).to.exist;
  });
  return describe('dimensions options', function() {
    it('exact width can be passed as option', function() {
      canvas = makeCanvas({
        width: 300
      });
      return expect(canvas.width).to.equal(300);
    });
    it('exact height can be passed as option', function() {
      canvas = makeCanvas({
        height: 500
      });
      return expect(canvas.height).to.equal(500);
    });
    it('auto width can be passed as option', function() {
      canvas = makeCanvas({
        width: 'auto'
      });
      return expect(canvas.width).to.equal(document.body.clientWidth);
    });
    return it('auto height can be passed as option', function() {
      canvas = makeCanvas({
        height: 'auto'
      });
      return expect(canvas.height).to.equal(document.body.clientHeight);
    });
  });
});
...
describe('KeyboardState', function() {
  ...
  triggerKeyEvent = function(type, key) {
    ...
    return keyboard.target.dispatchEvent(evt);
  };
  beforeEach(function() {
    return makeKeyboard();
  });
  it('can be created', function() {
    return expect(keyboard).to.exist;
  });
  it('has a target element and down object', function() {
    expect(keyboard.target).to.be["instanceof"](HTMLElement);
    expect(keyboard.down).to.be.an.object;
    return expect(keyboard.down).to.be.empty;
  });
  describe('target option', function() {
    return it('can target a different element', function() {
      makeKeyboard({
        target: '#mocha'
      });
      return expect(keyboard.target).to.have.property('id', 'mocha');
    });
  });
  describe('keymap option', function() {});
  return describe('key events', function() {
    return it('should track keydown', function() {
      triggerKeyEvent('keydown', 'q', 81);
      return expect(keyboard.down[81]).to.be["true"];
    });
  });
});
